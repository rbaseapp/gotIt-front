...
export function practiceQueue(items: LearningItem[], game: GameType, ids?: string[]): LearningItem[] {
  const active = items.filter(item => !item.deletedAt && item.userStatus === 'ACTIVE' && (!ids || ids.includes(item.id)));
  ...
  const first = sorted[0]; const sources = new Set<string>(); const meanings = new Set<string>();
  return sorted.filter(item => {
    if (item.sourceLanguage !== first.sourceLanguage || item.translationLanguage !== first.translationLanguage) return false;
    const source = normalizeAnswer(item.source); const meaning = normalizeAnswer(item.translation);
    if (sources.has(source) || meanings.has(meaning)) return false;
    sources.add(source); meanings.add(meaning); return true;
  }).slice(0, 4);
}
...
export function answerQuality(answer: string, expected: string): number {
  const a = normalizeAnswer(answer); const b = normalizeAnswer(expected);
  if (a === b) return 100;
...
export const gameSkills: Record<GameType, SkillKey[]> = {
  smart: ['recognition', 'recall'], flashcards: ['recognition'], recall: ['recall'],
  listening: ['listening', 'spelling'], matching: ['recognition'], pronunciation: ['pronunciation'],
};
