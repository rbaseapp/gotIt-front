...
export function demoAward(attempts: Attempt[], attempt: Attempt): number {
  ...
  if (
    ...
    attempt.game === "pronunciation"
  )
    return 0;
  // Illustrative demo XP only, not the production XP curve. One award per item/game/day.
  return attempts.some(
    (value) =>
      value.itemId === attempt.itemId &&
      value.game === attempt.game &&
      value.xp &&
      value.createdAt.slice(0, 10) === attempt.createdAt.slice(0, 10),
  )
    ? 0
    : 10;
}
export function demoReducer(state: DemoState, action: DemoAction): DemoState {
  ...
  if (state.attempts.some((attempt) => attempt.id === action.attempt.id))
    return state;
  const attempt = {
    ...action.attempt,
    xp: demoAward(state.attempts, action.attempt),
  };
  return {
    ...state,
...
const validDate = (value: unknown) =>
  typeof value === "string" && Number.isFinite(+new Date(value));
  ...
  return (
    ...
    state.items.every(
      (item) =>
        ...
        typeof item.translationLanguage === "string" &&
        validDate(item.createdAt) &&
        validDate(item.dueAt) &&
        (!item.deletedAt || validDate(item.deletedAt)) &&
        ["NEW", "LEARNING", "REVIEWING", "MASTERED"].includes(item.status) &&
        ["ACTIVE", "PAUSED", "ARCHIVED"].includes(item.userStatus) &&
        ...
        [
          ...
          const score = item.skills[key as keyof typeof item.skills];
          return Number.isFinite(score) && score >= 0 && score <= 100;
        }) &&
        (!item.examples || strings(item.examples)) &&
        (!item.translations || strings(item.translations)) &&
        (!item.occurrences ||
          (Array.isArray(item.occurrences) &&
            item.occurrences.every(
              (part) =>
                typeof part.context === "string" && validDate(part.createdAt),
            ))),
    ) &&
    ...
    state.attempts.every(
      (attempt) =>
        ...
        games.includes(attempt.game) &&
        validDate(attempt.createdAt) &&
        Number.isFinite(attempt.score) &&
        attempt.score >= 0 &&
        attempt.score <= 100 &&
        typeof attempt.correct === "boolean" &&
        (attempt.xp === undefined ||
          (Number.isFinite(attempt.xp) && attempt.xp >= 0)),
    ) &&
    ...
    state.sessions.every(
      (session) =>
        ...
        (!session.endedAt || validDate(session.endedAt)) &&
        games.includes(session.game) &&
        ["active", "completed", "abandoned"].includes(session.status) &&
        strings(session.itemIds) &&
        Number.isFinite(session.durationSeconds) &&
        session.durationSeconds >= 0 &&
        Number.isFinite(session.xp) &&
        session.xp >= 0,
    ) &&
    !!state.profile &&
