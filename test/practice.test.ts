...
describe("practice and demo evidence", () => {
  it("excludes paused, archived and deleted words, respects targeted selection without mutating the source", () => {
    const items = structuredClone(seedItems);
    const before = structuredClone(items);
    expect(
      practiceQueue(items, "recall", ["wander"]).map((item) => item.id),
    ).toEqual(["wander"]);
    expect(items).toEqual(before);
    items[0].deletedAt = new Date().toISOString();
    items[1].userStatus = "PAUSED";
    items[2].userStatus = "ARCHIVED";
    expect(
      practiceQueue(items, "smart").every(
  ...
  it("records each evidence event once without manufacturing mastery or review dates", () => {
    const initial = state();
    const event = attempt();
    const next = demoReducer(initial, { type: "attempt", attempt: event });
    expect(next.attempts).toHaveLength(1);
    expect(next.attempts[0].xp).toBe(10);
  ...
  it("soft delete and restore preserve attempts and accepted translations", () => {
    ...
    next = demoReducer(next, {
      type: "patch",
      id: "wander",
      patch: { deletedAt: "2026-09-15T10:10:00Z" },
    });
    ...
    expect(next.attempts).toHaveLength(1);
    expect(next.items[0].translation).toBe(seedItems[0].translation);
  ...
  it("upserts measured sessions and rejects malformed persisted demo data", () => {
    ...
    expect(
      demoReducer(next, {
        type: "session",
        session: { ...session, status: "completed", durationSeconds: 62 },
      }).sessions,
    ).toHaveLength(1);
